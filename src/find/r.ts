...
export default (context: Context, args: {
  ...
  showStale?: boolean,      // Whether to show results which are considered stale (IE: those over 12 hzours old)
  ...
  try {
    ...
    if (args) {
      ...
      const invalids = filters.filter((e) => !["limit", "page", "gibson", "corpsOnly", "description", "usage", "metrics", "user", "open", "tag", "tags", "name", "report", "showStale", "level", "sector", "publics", "prefix", "postfix", "regex"].includes(e))
    ...
    const { corps } = query_db("f", {}, { __corpdata: true })[0];
    ...
    const response = query_db("f", {}, filter, args.limit, undefined, args.page) as any[];
    const matches = query_db("c", {}, filter) as number;
    const total_pages = Math.ceil(matches / args.limit);
    const next_page = args.page + 1;

    const _metrics = query_db("f", {}, { __metrics: true })[0];
    ...
    const sector_info = Object.keys(sectors).map((e) => ({ sector: e, scripts: sectors[e] }));
    for (let i = 0; i < sector_info.length; i++) {
      ...
      if (sector_info.length === 1 && sector.scripts.length === 1) {
        ...
        const name = script.ikey.replace("#", ".");
        ...
        l.log(
          `Name:                ${name}\n`
          + `Author:              ${script.author !== "Uknown" ? script.author : (name.split(".")[0] || "Unknown")}\n`
