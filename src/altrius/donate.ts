...
export default (context: Context, args: {
    ...
    if (args.donateSource) return $ms.fatalcenturion.donate_4_me({ donate: 0, source: true, is_script: true });

    if (args.donate) {
        // if the donate argument is present, process...
        let dono = $ms.fatalcenturion.donate_4_me({
        ...
        if (dono.ok) {
            ...
            l.log("You may see the user `2fatalcenturion` in your transactions. This is my library script")
    ...
    if (Object.keys(args).length === 0) {
        ...
        l.log(`\`4Donations: \`\`3https:\/\/github\.com\/altriusrs\/hackmud\/blob\/main\/src\/altrius\/donate.ts\``)
        l.log(`\`4Library: \`\`3https:\/\/github\.com\/altriusrs\/hackmud\/blob\/main\/src\/fatalcenturion\/donate_4_me.ts\``)
